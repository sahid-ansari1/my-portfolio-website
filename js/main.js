...
class PortfolioManager {
  ...
  _buildTabMaps() {
    document.querySelectorAll('.tab-panel').forEach(panel => {
      this._tabPanels.set(panel.id, panel);
    });
    ...
    this._revealObserver = new IntersectionObserver(
      entries => {
        entries.forEach(entry => {
          ...
          this._revealObserver.unobserve(entry.target);  // stop observing
  ...
  /**
   * Register element with observer — Set prevents duplicates
   * @param {HTMLElement} el
   */
  _observe(el) {
    if (this._observed.has(el)) return;   // O(1) Set lookup
